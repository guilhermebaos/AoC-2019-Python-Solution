...
Wire1_Coords = wire_coor(Wire1_Puzzle)

Wire2_Coords = wire_coor(Wire2_Puzzle)

Wire1_Order = Wire1_Coords[:]
Wire2_Order = Wire2_Coords[:]
...
print(Wire1_Coords[0:20])
print(Wire2_Coords[0:20])

Intercections, Dists = [], []

# Problems may occur when taking out items os a list while iterating through it
c1 = -1
l1, l2 = len(Wire1_Coords), len(Wire2_Coords)

for coord1 in range(0, l1):
    desvio = 0
    c1 += 1
    # print(c1)
    for coord2 in range(0, l2):
        coord2 = coord2 + desvio
        print(coord1, coord2, l2)
        if l2 == 1:
            break
        if Wire1_Coords[coord1][0] == Wire2_Coords[coord2][0]:
            if Wire1_Coords[coord1][1] == Wire2_Coords[coord2][1]:
                Intercections += [Wire1_Coords[coord1]]
                Dists += [abs(Wire1_Coords[coord1][0]) + abs(Wire1_Coords[coord1][1])]
                print(abs(Wire1_Coords[coord1][0]) + abs(Wire1_Coords[coord1][1]))
            elif Wire1_Coords[coord1][1] < Wire2_Coords[coord2][1]:
                break
            else:
                Wire2_Coords.pop(coord2)
                desvio -= 1
                l2 -= 1
        elif Wire1_Coords[coord1][0] < Wire2_Coords[coord2][0]:
            break
        else:
            Wire2_Coords.pop(coord2)
            coord2 -= 1
...
Time = []
for coord in Intercections:
    t1 = Wire1_Order.index(coord)
    t2 = Wire2_Order.index(coord)
    Time += [t1 + t2]
    print('\n' + str(Time))
    print(t1, t2, coord)

print('\n' + str(min(Time)))
print(len(Intercections))
print(Intercections)